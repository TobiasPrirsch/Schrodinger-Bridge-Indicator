...
class Runner():
    ...
    def sb_joint_train(self, opt):
        ...
        ts = self.ts       #100     分割数
        batch_x = opt.samp_bs   #1000
        loss_doc = []
        zf_doc = []

        for it in range(opt.num_itr):      #2000
            ...
            ts_f = ts.repeat(batch_x)

            loss, z_f = compute_sb_nll_joint_train(
                opt, batch_x, self.dyn, ts_f, xs_f, zs_f, x_term_f, policy_b
            )
            ...
            if (it+1) % opt.eval_itr == 0 or it == 0:  #200
                with torch.no_grad():
                    ...
                    xs_f, _, x_term = self.dyn.sample_traj(ts, policy_f, save_traj=True)
                    xs_b1, _, _ = self.dyn.sample_traj(ts, policy_b, x_term, save_traj=True)
                util.save_toy_npy_traj(opt, 'train_itb{}'.format(it+1), xs_b.detach().cpu().numpy())
                util.save_toy_npy_traj(opt, 'train_itf{}'.format(it+1), xs_f.detach().cpu().numpy())
                util.save_toy_npy_traj(opt, 'train_itbb{}'.format(it+1), xs_b1.detach().cpu().numpy())
