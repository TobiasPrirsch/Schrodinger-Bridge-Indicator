...
class BaseSDE(metaclass=abc.ABCMeta):
    ...
    def dw(self, x, dt=None):
        ...
        return torch.randn_like(x)*np.sqrt(dt) * torch.tensor([1, 0])
    ...
    def propagate(self, t, x, z, direction, f=None, dw=None, dt=None):
        ...
        return x + (f + g*z * torch.tensor([5e-1, 5e-3]))*dt + g*dw
    ...
    def sample_traj(self, ts, policy, x_term_f=None, corrector=None, apply_trick=True, save_traj=True):
        ...
        init_dist = self.p if direction == 'forward' else self.q        #p=pdata q=prior
        ts = ts if direction == 'forward' else torch.flip(ts, dims=[0])
        if x_term_f == None:
            x = init_dist.sample()  # [bs, x_dim]   pdata
        else:
            x = x_term_f
