...
import policy
import sde
from loss import compute_sb_nll_alternate_train, compute_sb_nll_joint_train, loss_joint_train
import data
import util
...
class Runner():
    def __init__(self, opt):
        ...
        self.target = np.load('./results/experiments/gau3000_target_right.npy')
        self.target = torch.from_numpy(self.target).to(opt.device)

        # build dynamics, forward (z_f) and backward (z_b) policies
        self.dyn = sde.build(opt, self.p, self.q)            #VESDE
    ...
    def sb_alternate_train_stage(self, opt, stage, epoch, direction, reused_sampler=None):
        ...
        for ep in range(epoch):
            ...
            # evaluate
                ...
                # util.save_toy_npy_traj(opt, 'train_itf{}'.format(it + 1), xs_f.detach().cpu().numpy())
    ...
    def sb_alternate_train_ep(
        ...
        loss_doc = []
        for it in range(opt.num_itr):   #250
            ...
            loss_doc.append(loss.detach().cpu().numpy())
            ...
            if (it + 1) % opt.eval_itr == 0 or ep == 0:  # 200
                print(ep)
                print((it + 1) % opt.eval_itr)

                util.save_toy_npy_traj(opt, f'{direction} + {stage + 1} + {it + 1}', train_xs.detach().cpu().numpy())

        loss_npy = os.path.join('results', opt.dir, f'{direction}_{stage}_loss.npy')
        np.save(loss_npy, loss_doc)
    ...
    def sb_alternate_train(self, opt):
        for stage in range(opt.num_stage):    #8
            forward_ep = backward_ep = opt.num_epoch      #1
            print(backward_ep)
            ...
            n_reused_trajs = forward_ep * opt.samp_bs if opt.reuse_traj else 0      #0
    ...
    def sb_joint_train(self, opt):
        # print(util.is_image_dataset(opt))      #False
        assert not util.is_image_dataset(opt)

        policy_f, policy_b = self.z_f, self.z_b         #forward    backward
        policy_f = activate_policy(policy_f)      #激活网络参数
        ...
        for it in range(opt.num_itr):
            ...
            loss, cost, z_f, z_b, div_b = compute_sb_nll_joint_train(
                opt, batch_x, self.dyn, _ts, xs_f, zs_f, x_term_f, policy_b, policy_f, self.target
            )
            ...
            loss_doc.append(loss.detach().cpu().numpy())
            cost_doc.append(cost.detach().cpu().numpy())
            zf_doc.append(z_f.detach().cpu().numpy())
            ...
            # evaluate
            if (it+1) % opt.eval_itr == 0 or it == 0:  #200
                with torch.no_grad():
                    xs_b, _, _ = self.dyn.sample_traj(ts, policy_b, save_traj=True)
                    xs_f, _, _ = self.dyn.sample_traj(ts, policy_f, save_traj=True)
                util.save_toy_npy_traj(opt, 'train_itb{}'.format(it+1), xs_b.detach().cpu().numpy())
                util.save_toy_npy_traj(opt, 'train_itf{}'.format(it+1), xs_f.detach().cpu().numpy())
        loss_npy = os.path.join('results', opt.dir, 'loss.npy')
        cost_npy = os.path.join('results', opt.dir, 'cost.npy')
        zf_npy = os.path.join('results', opt.dir, 'zf.npy')

        np.save(loss_npy, loss_doc)
        np.save(cost_npy, cost_doc)
        np.save(zf_npy, zf_doc)
    ...
    def log_sb_joint_train(self, opt, it, loss, optimizer, num_itr):
        self._print_train_itr(it, loss, optimizer, num_itr, name='SB joint')   #print training process
        if opt.log_tb:     #tensorboard
            step = self.update_count('backward')
            self.log_tb(step, loss.detach(), 'loss', 'SB_joint')   #add_scalar
